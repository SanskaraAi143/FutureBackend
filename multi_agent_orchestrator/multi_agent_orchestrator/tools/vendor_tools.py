...
async def list_vendors(tool_context: ToolContext,filters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    ...
    Example Usage:
        ```python
        response = await list_vendors({"vendor_category": "Photographer", "city": "Goa"}, tool_context)
    ...
    try:
        ...
        if isinstance(result, list):
            ...
            logger.info(f"list_vendors: Successfully retrieved {len(result)} vendors. Cached: {bool(tool_context)}")
...
async def get_vendor_reviews(vendor_id: str, tool_context: ToolContext) -> Dict[str, Any]:
    """
    ...
    Example Usage:
        ```python
        response = await get_vendor_details("some-vendor-uuid", tool_context)
        if response["status"] == "success":
    ...
    try:
        ...
        if vendor_data:
            if tool_context:
                tool_context.state[cache_key] = vendor_data # Cache the result
            logger.info(f"get_vendor_details: Successfully retrieved details for vendor_id {vendor_id}. Cached: {bool(tool_context)}")
...
async def search_vendors(
    ...
    Example Usage:
        ...
        response = await search_vendors(**search_criteria, tool_context=tool_context)
        if response["status"] == "success":
    ...
    try:
        ...
        if isinstance(result, list):
            if tool_context:
                tool_context.state[cache_key] = result # Cache the result
            logger.info(f"search_vendors: Successfully retrieved {len(result)} vendors matching criteria. Cached: {bool(tool_context)}")
            return {"status": "success", "data": result}
        else:
...
async def check_vendor_availability(vendor_id: str, date: str, tool_context: ToolContext) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if tool_context:
            tool_context.state[cache_key] = {"available": is_available} # Cache the result
        logger.info(f"Vendor {vendor_id} on {date} is {'available' if is_available else 'not available'}. Cached: {bool(tool_context)}")
...
async def update_vendor_status(vendor_id: str, status: str, tool_context: ToolContext) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if isinstance(result, list) and result:
            ...
            if tool_context:
                ...
                if f"vendor_details:{vendor_id}" in tool_context.state:
                    del tool_context.state[f"vendor_details:{vendor_id}"]
            ...
            logger.info(f"Successfully updated status for vendor {vendor_id}. Cache invalidated: {bool(tool_context)}")
