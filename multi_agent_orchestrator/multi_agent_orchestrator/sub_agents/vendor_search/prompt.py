# Prompts for vendor search sub-agent
...
VENDOR_PROMPT = (
    ...
    "If no vendors are found that match the user's criteria, respond with a clear and helpful message, such as 'No vendors found for your specified criteria.'"
    "Start with broad search criteria and then narrow down based on user input. If the initial search yields too many results, ask the user for more specific criteria."
    "When selecting tools, prioritize specialized tools over generic ones. For example, if searching for a specific vendor detail, use the 'get_vendor_details' tool instead of the 'list_vendors' tool."
)
